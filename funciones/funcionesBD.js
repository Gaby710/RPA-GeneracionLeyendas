...
async function actualizaProcesoTerminadoDAT(taskname, numleyendas, fechasql, estado, bodymail) {
    return new Promise((resolve, reject) => {
        pool.getConnection((error, connection) => {
            ...
            console.log('query de update tarea', query);

            connection.query(query, [numleyendas, fechasql, estado, bodymail, taskname], (error, results) => {
                connection.release();
...
async function generaFechaSQLUpdate() {
    var fechaSql = new Date();
    //onsole.log('new date', fechaSql)
    fechaSql.setHours(fechaSql.getHours()-6);
    fechaSql= fechaSql.toJSON().replace('T', ' ').replace('Z','')
    console.log('updateFecha', fechaSql)
    return fechaSql;
}
...
module.exports.generaFechaSQLUpdate = generaFechaSQLUpdate;