...
dotenv.config();
const logger = require('node-logger').createLogger('logs_ConveniosPruebas.txt');
...
async function cortarLeyendaDESCBRIEF(valor, habitacionleyenda, precioleyendax, desayunoleyendax, flagBKFST, flagdolar, flaglocal, flagcorp, descripciones, unicoshot, flagALLINCLUSIVE, flagEspecial, flaginvertir) {
    ...
    if (leyendaDESCBRIEF.length > 30) {
        ...
        leyendaDESCBRIEF = leyendaDESCBRIEF + 'MX';
        if ((flaglocal && flagBKFST) || unicoshot.includes('1') || unicoshot.includes('FX') || unicoshot == 'FICCL' || (flagcorp && flagBKFST)) {
            leyendaDESCBRIEF = leyendaDESCBRIEF + ' BKFST';
        }
    }
...
async function plancharPlantilla(carga, taskname) {
    ...
    try {
        ...
        let rutaarchivo = path.join(process.env.PLANTILLAS_CARGA, 'PR'+taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '.csv');
        if (!fs.existsSync(rutaarchivo)) {
            fs.closeSync(fs.openSync(path.join(process.env.PLANTILLAS_CARGA, 'PR'+taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '.csv'), 'w'));
            fs.closeSync(fs.openSync(path.join(process.env.HISTORIAL_PLANTILLAS, 'PR'+taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '.csv'), 'w'));
            const csv = new ObjectsToCsv(carga);
            csv.toDisk(path.join(path.join(process.env.PLANTILLAS_CARGA, 'PR'+taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '.csv')));
            csv.toDisk(path.join(path.join(process.env.HISTORIAL_PLANTILLAS, 'PR'+taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '.csv')));

            console.log('Files created: ' + taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '.csv');
            return taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '.csv';
        } else {
            let contador = 1;
            rutaarchivo = path.join(process.env.PLANTILLAS_CARGA, 'PR'+taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '_' + String(contador) + '.csv');
            while (fs.existsSync(rutaarchivo)) {
                contador++;
                rutaarchivo = path.join(process.env.PLANTILLAS_CARGA, 'PR'+ taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '_' + String(contador) + '.csv');
            }
            fs.closeSync(fs.openSync(path.join(process.env.PLANTILLAS_CARGA, 'PR'+taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '_' + String(contador) + '.csv'), 'w'));
            fs.closeSync(fs.openSync(path.join(process.env.HISTORIAL_PLANTILLAS, 'PR'+taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '_' + String(contador) + '.csv'), 'w'));
            const csv = new ObjectsToCsv(carga);
            csv.toDisk(path.join(path.join(process.env.PLANTILLAS_CARGA, 'PR'+taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '_' + String(contador) + '.csv')));
            csv.toDisk(path.join(path.join(process.env.HISTORIAL_PLANTILLAS, 'PR'+taskname + ' ' + dia[0] + '-' + dia[1] + '-' + dia[2] + '_' + String(contador) + '.csv')));
